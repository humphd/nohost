// Base64 entries for all PNG icon files.  Built using:
// `npx datauri-cli filename`.  Initial, common portion of
// data URI and firist few bytes of PNG data are stripped,
// see below when reconstructed. 
const base64Images = {
  back: '',
  blank: '',
  folder: '',
  image2: '',
  movie: '',
  text: '',
  unknown: ''
};

module.exports = {
  getDataUri: function(name) {
    const imgData = base64Images[name];
    if(!imgData) {
      throw new Error(`Invalid icon name: ${name}`);
    }

    return `data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABQAAAAW${imgData}`;
  }
};
