/**
* Based on XMLHttpRequest.js - https://github.com/ilinsky/xmlhttprequest
* Copyright (C) 2011 Sergey Ilinsky (http://www.ilinsky.com)
*
...
define(['filer'], function(Filer) {
  ...
  // A fake request for cases when data comes from the filesystem
  var nohostRequests = (function() {
    var requests = {};

    function NohostRequest(id) {
      this.id = id;
      requests[id] = this;
    }
    NohostRequest.prototype.done = function(responseText, dispatchEvent) {
    ...
    return {
      create: function(id, responseText, dispatchEvent) {
        return new NohostRequest(id, responseText, dispatchEvent);
      },
      get: function(id) {
        return requests[id];
      }
  ...
  // Save reference to earlier defined object implementation (if any)
	...
	cXMLHttpRequest.prototype.open  = function(sMethod, sUrl, bAsync, sUser, sPassword) {
      ...
      // Check to see if we have a request from nohost, and use that
      oRequest = nohostRequests.get(this._id) || oRequest;
    ...
    // XXXnohost: try to read this url out of the filesystem
    var nohostURL = location.search.substring(1);
    if (nohostURL.indexOf('//') === -1) {
      // We'll fill this object in later, but we need to shortcut other operations
      // that will look for it to exist.
      var request = nohostRequests.create(this._id);

		  this.readyState = cXMLHttpRequest.OPENED;
		  fReadyStateChange(this);

      nohost.rewriteURL(nohostURL, 'utf8', function(err, data) {
        // XXX deal with err case
        if(err) {
          console.log('err', err);
        }

        request.done(data, onreadystatechange);
		    fReadyStateChange(request);
      });
    } else { // normal network load
    ...
    this._object.onreadystatechange = onreadystatechange;
	};
    ...
    if(nohostRequests.get(this._id)) {
      return;
    }
